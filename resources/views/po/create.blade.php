@extends('layouts.app')
@section('title', 'Input Transaksi Pembelian Barang')
    ...
    <div class="po-body">
        ...
        <div class="panel-left">
            ...
            <div class="panel-form">
                ...
                <div class="frow">
                    <label>Nomor Data @if($isDraft)<span id="nomor-badge" class="nomor-badge {{ $jenisAwal==='PO'?'po':'pr' }}">{{ $jenisAwal==='PO'?'PO':'PR' }}</span>@endif</label>
                    <input type="text" name="po_number" id="po_number" class="fc ro" value="{{ $po->po_number }}" readonly>
                </div>

                <div class="frow">
                    <label>Tanggal</label>
                    <input type="date" name="tanggal" id="tgl_transaksi" class="fc {{ !$isDraft?'ro':'' }}"
                           value="{{ $po->tanggal?->format('Y-m-d') ?? date('Y-m-d') }}" {{ !$isDraft?'readonly':'' }}>
                </div>
                ...
                <div class="frow" id="row-faktur">
                    <label>Faktur</label>
                    <input type="text" name="nomor_faktur" class="fc {{ !$isDraft?'ro':'' }}"
                           value="{{ $po->nomor_faktur }}" {{ !$isDraft?'readonly':'' }} placeholder="No. Faktur Supplier">
                </div>
                ...
                <div class="frow" id="row-tgl-faktur">
                    ...
                    <input type="date" name="tanggal_faktur" id="tgl_faktur"
                           class="fc {{ !$isDraft?'ro':'' }}"
                           value="{{ $po->tanggal_faktur?->format('Y-m-d') }}" {{ !$isDraft?'readonly':'' }}>
                </div>
                ...
                <div class="frow" id="row-jk">
                    ...
                    <div class="jk-wrap">
                        <input type="number" name="jk_waktu" id="jk_waktu" class="fc-num"
                               value="{{ $po->jk_waktu ?? 0 }}" min="0" {{ !$isDraft?'readonly':'' }}>
                        <span>hari</span>
                ...
                <div class="frow" id="row-bulan-lapor">
                    ...
                    <input type="month" name="bulan_lapor" class="fc {{ !$isDraft?'ro':'' }}"
                           value="{{ $po->bulan_lapor }}" {{ !$isDraft?'readonly':'' }}>
                </div>
                    ...
                    <table class="table table-sm table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr class="{{ $r->id==$po->id?'table-primary fw-bold':'' }}"
                                ...
                                <td>{{ $r->po_number }}</td>
                                <td style="max-width:80px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis">{{ $r->supplier->nama_supplier ?? '-' }}</td>
                                <td>@php $bc=match($r->status){'approved'=>'primary','received'=>'success','canceled'=>'danger',default=>'secondary'}; @endphp
                                    <span class="badge bg-{{ $bc }}" style="font-size:0.63rem">{{ ucfirst($r->status) }}</span></td>
                            </tr>
                            @empty
        ...
        <div class="panel-right">
            <div class="panel-right-hdr">
                <span>
                    ...
                    {{ $po->po_number }}
                    @if($po->supplier)<span class="fw-normal" style="opacity:.75"> — {{ $po->supplier->nama_supplier }}</span>@endif
                </span>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            @if($isDraft)<th width="32"></th>@endif
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($po->items as $i => $item)
                        @php $sub = $item->qty * $item->price; @endphp
                        <tr>
                            ...
                            <td style="text-align:center">
                                <form method="POST" action="{{ route('po.deleteItem', $item->id) }}" onsubmit="return confirm('Hapus item ini?')">
                                    @csrf @method('DELETE')
                                    <button class="btn btn-link text-danger p-0" style="font-size:0.82rem"><i class="bi bi-x-circle-fill"></i></button>
                                </form>
                            </td>
                            @endif
                        </tr>
                        @empty
                        <tr><td colspan="9" style="text-align:center;color:#aaa;padding:24px 0">
                            <i class="bi bi-inbox" style="font-size:1.5rem;display:block;margin-bottom:4px;opacity:.4"></i>Belum ada item
                        </td></tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="bottom-area">
                ...
                <div class="item-input-area">
                    ...
                    @csrf
                    <div class="form-section-title"><i class="bi bi-plus-circle me-1 text-primary"></i>Tambah Item Barang</div>
                    ...
                    <div class="irow">
                        ...
                        <select name="product_unit_id" id="sel_produk" class="ic" onchange="onProdukChange(this)" required>
                            ...
                            <option value="{{ $unit->id }}" data-satuan="{{ $unit->unit_name ?? 'pcs' }}" data-harga="{{ $unit->price ?? 0 }}">
                                {{ $unit->product->name ?? '-' }} @if($unit->unit_name)({{ $unit->unit_name }})@endif
                            </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="irow-split">
                        <label>Jumlah</label>
                        <input type="number" name="qty" id="input_qty" class="ic" value="1" min="1" step="1" oninput="hitungSubtotal()" required>
                        <span></span>
                        <input type="text" id="preview_satuan" name="satuan" class="ic" style="background:#e9ecef" readonly placeholder="Satuan">
                    </div>

                    <div class="irow">
                        <label>Harga Satuan</label>
                        <input type="number" name="price" id="input_price" class="ic" value="0" min="0" step="1" oninput="hitungSubtotal()" required>
                    </div>

                    <div class="irow-split">
                        <label>Diskon Barang</label>
                        <input type="number" name="diskon_persen" id="input_diskon" class="ic" value="0" min="0" max="100" step="0.1" oninput="hitungSubtotal()">
                        <span>%</span>
                        <input type="number" id="preview_diskon_rp" class="ic" style="background:#e9ecef;text-align:right" readonly value="0" placeholder="Rp diskon">
                    </div>

                    <div class="irow">
                        <label>Sub Total</label>
                        <input type="text" id="preview_subtotal" class="ic" style="background:#fff3cd;font-weight:700;text-align:right;border-color:#ffc107" readonly value="0">
                    </div>
                    ...
                    <button type="submit" class="btn-tambah"><i class="bi bi-plus-lg me-1"></i>Tambah Item ke PO</button>
                    </form>
                    @else
                ...
                <div class="summary-area">
                    ...
                    <div class="srow"><label>Total Qty</label><input class="sv" readonly value="{{ number_format($totalQty,0,',','.') }}"></div>
                    <div class="srow"><label>Total HNA</label><input class="sv" readonly value="Rp {{ number_format($grandTotal,0,',','.') }}"></div>
                    <div class="srow"><label>PPN ({{ $po->ppn??0 }}%)</label><input class="sv" readonly value="Rp {{ number_format($ppnRp,0,',','.') }}"></div>
                    <div class="srow"><label>Jenis Bayar</label><input class="sv" readonly value="{{ $po->jenis_pembayaran ?? '-' }}"></div>
                    <div class="srow"><label>Jatuh Tempo</label><input class="sv" readonly value="{{ $po->tanggal_jatuh_tempo?->format('d/m/Y') ?? '-' }}"></div>
                    ...
                    <div style="margin-top:auto;padding-top:8px;display:flex;flex-direction:column;gap:4px">
                        ...
                        <form method="POST" action="{{ route('po.approve', $po->id) }}" onsubmit="return confirm('Approve & kunci PO ini?')">
                            @csrf
                            <button class="btn btn-success w-100" style="font-size:0.75rem;padding:4px 8px;border-radius:2px">
                        ...
                        <form method="POST" action="{{ route('po.cancel', $po->id) }}" onsubmit="return confirm('Batalkan PO ini?')">
                            @csrf
                            <button class="btn btn-outline-danger w-100" style="font-size:0.75rem;padding:4px 8px;border-radius:2px">
                        ...
                        <form method="POST" action="{{ route('po.receive', $po->id) }}" onsubmit="return confirm('Tandai barang sudah diterima?')">
                            @csrf
                            <button class="btn btn-primary w-100" style="font-size:0.75rem;padding:4px 8px;border-radius:2px">
                                <i class="bi bi-box-arrow-in-down me-1"></i>Terima Barang
                            </button>
                        </form>
                        @endif
                        <a href="{{ route('po.index') }}" class="btn btn-outline-secondary w-100" style="font-size:0.75rem;padding:4px 8px;border-radius:2px">
...
// ★ Auto-isi Jk. Waktu = 30 hari saat Tanggal Faktur dipilih & Kredit
...
document.addEventListener('DOMContentLoaded', function() {
    if (isDraft && jenisSelect) { togglePORows(jenisSelect.value); updateNomor(jenisSelect.value); }
    syncRowJK();
    hitungJatuhTempo();
    hitungSubtotal();
});
