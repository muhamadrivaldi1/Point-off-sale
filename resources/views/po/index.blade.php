        ...
        <div class="card-body bg-light border-bottom">
            <form method="GET" action="{{ route('po.index') }}" class="row g-3">
                <div class="col-md-2">
                    ...
                    <input type="text" name="cari" class="form-control form-control-sm" value="{{ request('cari') }}">
                </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ ($pos->currentPage() - 1) * $pos->perPage() + $index + 1 }}</td>
                        ...
                        <td>{{ $po->tanggal_jatuh_tempo ? \Carbon\Carbon::parse($po->tanggal_jatuh_tempo)->format('d/m/Y') : '-' }}</td>
                        <td class="text-end fw-bold">Rp {{ number_format($po->total, 0, ',', '.') }}</td>
                        ...
                        <td class="text-center">
                            <div class="d-flex justify-content-center gap-1">
                                ...
                                <a href="{{ route('po.edit', $po->id) }}" class="btn btn-sm btn-outline-primary" title="Edit Data">
                                ...
                                @if($po->status == 'draft')
                                    <form action="{{ route('po.approve', $po->id) }}" method="POST">
                                        @csrf
                                        <button class="btn btn-sm btn-outline-info" title="Approve">
                                ...
                                @if($po->status == 'approved')
                                    <form action="{{ route('po.receive', $po->id) }}" method="POST">
                                        @csrf
                                        <button class="btn btn-sm btn-outline-success" title="Terima Barang">
                                ...
                                @if($po->status == 'received')
                                    <a href="{{ route('purchase_returns.create', $po->id) }}" class="btn btn-sm btn-outline-warning" title="Retur Pembelian">
                                        <i class="bi bi-arrow-return-left"></i>
                                    </a>
                                @endif

                                {{-- Tombol Hapus --}}
                                <form action="{{ route('po.destroy', $po->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus PO ini?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="9" class="text-center py-5 text-muted">Belum ada data Purchase Order</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="card-footer bg-white">
            <div class="d-flex justify-content-between align-items-center">
                <div class="small text-muted">
                    Menampilkan {{ $pos->firstItem() ?? 0 }} - {{ $pos->lastItem() ?? 0 }} dari {{ $pos->total() }} data
                </div>
