...
{{-- ================= GLOBAL USER + SESSION + PERMISSION ================= --}}
@php
    $user = auth()->user();

    // Load permission sekali saja (hemat query & anti error)
    ...
    if (!function_exists('hasAkses')) {
        ...
        function hasAkses($permName) {
            ...
            // Kasir Full = full akses
            if ($u->role === 'kasir' && $u->kasir_level === 'full') return true;

            // Kalau tidak punya relasi permission → tolak
            if (!method_exists($u, 'directPermissions')) return false;

            // Load kalau belum
            if (!$u->relationLoaded('directPermissions')) {
                try {
                    $u->load('directPermissions');
                } catch (\Exception $e) {
                    return false;
                }
            }
