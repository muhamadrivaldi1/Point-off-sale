...
.readonly-banner-btn {
    ...
    background: #e67e00; color: #fff; border: none; border-radius: 6px;
...
/* Readonly: right panel subtle bg */
.pos-right.readonly-panel {
    background: #fffdf7;
    border-color: #ffe0b2;
}
...
/* ===== KREDIT READONLY SUMMARY PANEL ===== */
.kredit-readonly-summary {
...
.krs-sisa-row {
    display: flex; justify-content: space-between; align-items: center;
    background: #fff5f5; border: 1.5px solid #fca5a5; border-radius: 7px;
...
.btn-detail-kredit {
    ...
    width: 100%; padding: 10px; font-size: 14px; font-weight: 800; color: #fff;
    background: linear-gradient(135deg, #e67e00, #c45c00);
    border: none; border-radius: 8px; cursor: pointer;
    transition: all .15s; text-decoration: none; margin-top: 4px;
}
.btn-detail-kredit:hover { box-shadow: 0 4px 14px rgba(230,126,0,.35); transform: translateY(-1px); color: #fff; }

/* ===== KREDIT PANEL (non readonly) ===== */
.kredit-panel-full {
    background: linear-gradient(135deg, #fff8f0, #fff3e0);
...
#jurnalOverlay {
    ...
    z-index: 1060; display: none; align-items: stretch;
    justify-content: flex-end; backdrop-filter: blur(2px);
}
#jurnalOverlay.show { display: flex; }
#jurnalPanel {
    width: 82vw; max-width: 1100px; background: #fff;
    box-shadow: -6px 0 32px rgba(0,0,0,.22); display: flex;
...
.jr-close-btn {
    ...
    border-radius: 6px; padding: 5px 12px; cursor: pointer; font-size: 16px;
...
<div class="pos-wrapper">
    ...
    <div class="readonly-banner">
        ...
        <a href="{{ route('pos.kredit.show', $trx->id) }}" class="readonly-banner-btn">
            📋 Detail &amp; Bayar Kredit →
        </a>
    </div>
    @endif
    ...
    <div class="trx-header {{ $isReadOnly ? 'readonly-header' : '' }}">
        <div class="trx-left">
            ...
            @if($isReadOnly)
                <span class="readonly-badge">🔒 Kredit – Read Only</span>
            @endif
        </div>
        <div class="trx-right">
                    ...
                    <tbody id="searchResult">
                        <tr>
                            <td colspan="{{ 4 + count($warehouses) }}" class="text-center text-muted"
                                ...
                                @if($isReadOnly)
                                    🔒 Transaksi kredit — pencarian produk dinonaktifkan
                                @else
            ...
            <div style="flex-shrink:0;">
                ...
                @if($isReadOnly)
                    <input type="text" class="form-control form-control-xs input-readonly-mode mt-1"
                           value="{{ $trx->member ? $trx->member->name : 'Tidak ada member' }}"
                           readonly disabled>
                @else
                ...
                <div id="memberInfo" class="mt-1 member-info">
                    @if($isReadOnly && $trx->member)
                        <strong>Nama:</strong> {{ $trx->member->name }}
                        | <strong>Level:</strong> {{ $trx->member->level }}
                        | <strong>Disc:</strong> {{ $trx->member->discount }}%
                    @endif
                </div>
            </div>
                ...
                <table class="table table-sm pos-table mb-0">
                    ...
                    <tbody>
                        ...
                        <tr style="font-size:11px; cursor:pointer;"
                            @if($t->status === 'pending')
                                onclick="openPending({{ $t->id }})"
                                title="Klik untuk lanjutkan transaksi"
                            @elseif($t->status === 'kredit')
                                onclick="openKreditReadOnly({{ $t->id }})"
                                title="Klik untuk melihat detail kredit (read-only)"
                            @else
                                onclick="openPaidTransaction({{ $t->id }})"
                                title="Klik untuk buka kembali transaksi (butuh password)"
                            @endif
                        >
        ...
        <div class="pos-right {{ $isReadOnly ? 'readonly-panel' : '' }}">
            ...
            <div class="cart-footer">
                ...
                <div class="kredit-readonly-summary">
                    ...
                    @if($trx->debtor_name)
                    <div class="krs-row">
                        <span class="krs-label">👤 Peminjam</span>
                        <span class="krs-value">{{ $trx->debtor_name }}</span>
                    </div>
                    @endif

                    @if($trx->debtor_phone)
                    <div class="krs-row">
                        <span class="krs-label">📱 Telepon</span>
                        <span class="krs-value">{{ $trx->debtor_phone }}</span>
                    </div>
                    @endif

                    @if($trx->due_date)
                    <div class="krs-row">
                        <span class="krs-label">📅 Jatuh Tempo</span>
                        <span class="krs-value">{{ \Carbon\Carbon::parse($trx->due_date)->translatedFormat('d M Y') }}</span>
                    </div>
                    @endif

                    @if($trx->payment_plan)
                    <div class="krs-row">
                        <span class="krs-label">💳 Cara Bayar</span>
                        <span class="krs-value">{{ ucfirst($trx->payment_plan) }}{{ $trx->installment_count ? " ({$trx->installment_count}x)" : '' }}</span>
                    </div>
                    @endif

                    @if($trx->discount > 0)
                    <div class="krs-row">
                        <span class="krs-label">🏷️ Diskon</span>
                        <span class="krs-value" style="color:#28a745;">- Rp {{ number_format($trx->discount) }}</span>
                    </div>
                    @endif

                    <hr class="krs-divider">

                    <div class="krs-total-row">
                        <span class="krs-total-label">💰 Total Belanja</span>
                        <span class="krs-total-value">Rp {{ number_format($trx->total) }}</span>
                    </div>

                    @if($totalTerbayar > 0)
                    <div class="krs-row" style="margin-top:6px;">
                        <span class="krs-label" style="color:#065f46;">✅ Sudah Dibayar</span>
                        <span class="krs-value" style="color:#059669;">Rp {{ number_format($totalTerbayar) }}</span>
                    </div>
                    @endif

                    @if($sisaHutang > 0)
                    <div class="krs-sisa-row">
                        <span class="krs-sisa-label">📌 Sisa Hutang</span>
                        <span class="krs-sisa-value">Rp {{ number_format($sisaHutang) }}</span>
                    </div>
                    @else
                    <div class="krs-sisa-row" style="border-color:#a7f3d0; background:#f0fdf4;">
                        <span class="krs-sisa-label" style="color:#065f46;">✅ Sudah Lunas</span>
                        <span class="krs-sisa-value" style="color:#059669;">Rp 0</span>
                    </div>
                    @endif
                </div>

                <a href="{{ route('pos.kredit.show', $trx->id) }}" class="btn-detail-kredit">
                    📋 Buka Halaman Detail &amp; Pembayaran Kredit →
                </a>
...
const IS_READ_ONLY  = @json($isReadOnly);   // ← FLAG READONLY
...
function focusNext(currentId) {
    if (IS_READ_ONLY) return; // Jangan navigasi jika read-only
...
window.addEventListener('load', () => {
    ...
    const now = new Date();
    const tgPeriode = document.getElementById('tgPeriode');
    if (tgPeriode) tgPeriode.value = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;
});

function createNewTransaction() { window.location.href = '/pos?new_transaction=1'; }

/* ── Kredit READ-ONLY opener ──────────────────────────────── */
function openKreditReadOnly(trxId) {
    // Langsung navigasi ke POS dengan trx_id kredit — tampil read-only
    window.location.href = `/pos?trx_id=${trxId}`;
}

/* ── Unlock owner fields ──────────────────────────────────── */
function unlockMember() {
    if (IS_READ_ONLY) return; // Blokir jika read-only
...
function onMethodChange(method) {
    ...
    if (method === 'kredit') {
        ...
        if (memberEl && memberEl.value && memberEl.value.trim() !== '') { const kNama = document.getElementById('kreditNama'); if (!kNama.value) kNama.value = memberEl.value; }
...
if (!IS_READ_ONLY) {
    const kreditCaraBayar = document.getElementById('kreditCaraBayar');
    if (kreditCaraBayar) {
        kreditCaraBayar.addEventListener('change', function () {
...
function loadCart() {
    if (IS_READ_ONLY) return; // Jangan reload cart jika read-only
...
function plusQty(id) {
    if (IS_READ_ONLY) return;
    updateQtyManual(id, getQty(id) + 1);
}
function minusQty(id) {
    if (IS_READ_ONLY) return;
    updateQtyManual(id, Math.max(getQty(id) - 1, 1));
}
...
function updateUnit(itemId, unitId) {
    ...
    fetch('/pos/update-unit', { method:'POST', headers:jsonHeaders,
        ...
        if (res.readonly) { alert('🔒 ' + res.message); return; }
        loadCart();
    });
}
        ...
        .then(r => r.json()).then(r => {
            ...
            fetch('/pos/remove-item', { method:'POST', headers:jsonHeaders, body:JSON.stringify({ trx_id:TRX, item_id:itemId }) })
                .then(res => res.json()).then(res => {
                    if (res.readonly) { alert('🔒 ' + res.message); return; }
                    if (res.success) loadCart(); else alert("Gagal menghapus item.");
                });
        });
}
...
if (!IS_READ_ONLY) {
    ...
    if (paidEl) paidEl.addEventListener('input', updateKembalian);
}
...
function updateKembalian() {
    ...
    const bayar = Number(paidEl.value || 0);
    changeEl.innerText = 'Rp ' + Math.max(bayar - total, 0).toLocaleString('id-ID');
}
...
if (!IS_READ_ONLY) {
    ...
    if (memberEl) {
        memberEl.addEventListener('keyup', function (e) {
...
function selectMember(id) {
    ...
    fetch(`/pos/get-member?id=${id}`).then(r=>r.json()).then(m => {
        const el = document.getElementById('member');
        el.value = m.name; el.dataset.memberId = m.id;
        ...
        const kNama = document.getElementById('kreditNama'); if (kNama && !kNama.value) kNama.value = m.name;
        applyDiscountLive();
...
function getFinalDiscount() {
    const totalEl = document.getElementById('totalText');
    if (!totalEl) return 0;
    const total = Number(totalEl.dataset.original || 0); if (total <= 0) return 0;
...
function reloadJurnal() {
    ...
    loading.style.display = 'flex';
    frame.style.display   = 'none';
    frame.src             = JURNAL_URL + '?_t=' + Date.now();
}
